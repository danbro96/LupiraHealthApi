...
/// <summary>Owns the raw <c>telemetry</c> schema (tables + indexes), applied via the app's <c>--apply-schema</c> one-shot
/// after Marten's own apply. Marten's schema-diff only inspects the <c>health</c> schema, so it never touches these
/// tables. Ongoing partition create/drop is handled at runtime (ingest pre-creates on demand; the maintenance service
/// drops expired ones). All DDL is idempotent.</summary>
public static class TelemetrySchema
{
    ...
    public static async Task TruncateAllAsync(NpgsqlDataSource db, CancellationToken ct = default)
    {
        ...
        await using var cmd = new NpgsqlCommand(
            "TRUNCATE telemetry.location_point, telemetry.location_current, telemetry.ring_sample, telemetry.device_summary", conn);
        await cmd.ExecuteNonQueryAsync(ct);
    }
