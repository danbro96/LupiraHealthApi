...
/// <summary>Periodic maintenance for the telemetry store: provision upcoming partitions, roll up recent days into
/// Visits/Trips/DailyLocationSummary (freezing place labels), and drop expired raw-location partitions for retention.
/// Defensive + gated by <c>Telemetry:MaintenanceEnabled</c> (disabled in tests so it never races the per-test reset).</summary>
