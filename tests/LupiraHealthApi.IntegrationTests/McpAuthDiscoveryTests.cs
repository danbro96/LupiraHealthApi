...
public sealed class McpAuthDiscoveryTests(HealthApiTestFactory factory) : IntegrationTest(factory)
{
    [Theory]
    ...
    public async Task Metadata_is_anonymous_and_names_the_issuer(string path)
    {
        ...
        Assert.Equal(["https://auth.test/application/o/lupira-health/"], servers);
    ...
    {
        var anon = Factory.AnonymousClient();
        var resp = await anon.PostAsync("/mcp", null);
