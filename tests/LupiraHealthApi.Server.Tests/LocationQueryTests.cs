...
public sealed class LocationQueryTests(HealthApiTestFactory factory) : IntegrationTest(factory)
{
    [Fact]
    public async Task Current_filters_by_device_and_lists_all_without_filter()
    {
        ...
        var d1 = await RegisterDeviceAsync(api, record.Id, "Phone", "Phone");
        var d2 = await RegisterDeviceAsync(api, record.Id, "Watch", "Watch");
