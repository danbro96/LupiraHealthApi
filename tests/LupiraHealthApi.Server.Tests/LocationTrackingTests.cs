...
public sealed class LocationTrackingTests(HealthApiTestFactory factory) : IntegrationTest(factory)
{
    [Fact]
    public async Task Pause_then_state_reports_paused_with_reason()
    {
        ...
        Assert.Equal(HttpStatusCode.NoContent, (await api.PostAsJsonAsync($"/api/location/tracking/{deviceId}/pause", new PauseTrackingRequest("battery saver"))).StatusCode);
    ...
    public async Task Resume_clears_paused_and_reason()
    {
        var api = Factory.ApiClient("alice@x.test");
        var (_, _, deviceId) = await SetupDeviceAsync(api);
        await api.PostAsJsonAsync($"/api/location/tracking/{deviceId}/pause", new PauseTrackingRequest("x"));
